...
export function validateComputeResponse(body) {
  ...
  // Level must be consistent with the strongest category. If the endpoint
  // reports a level below its strongest returned category severity, clamp the
  // level UP to that severity — never down. A category we validated as sev N
  // means the consequence is at least level N, so raising keeps the response
  // internally consistent without weakening judgment. Downward adjustment is
  // never performed here.
  const maxSev = categories.reduce((m, c) => Math.max(m, c.sev), 0);
  const normalizedLevel = Math.max(level, maxSev);
  return { ok: true, classification: { level: normalizedLevel, categories } };
}
